...
export const getReactMarkDownCustomComponents = (messageIndex = 0, messageId = '') => {
  return useMemo(() => ({
    ...
    a: memo(({ href, children, ...props }) => (
      <a href={href} className="text-[#F47A8A] no-underline hover:underline" {...props}>
        {children}
      </a>
    ), (prevProps, nextProps) => isEqual(prevProps.children, nextProps.children)),
    ...
    sup: memo(({ children, ...props }) => {
      ...
      return validContent ? (
        <sup
          className="text-xs bg-gray-100 text-[#F47A8A] border border-[#e7ece0] px-1 py-0.5 rounded-md shadow-sm"
          style={{
            fontWeight: "bold",
            marginLeft: "2px",
