...
export const ChatHeader = ({ webSocketModeRef = {} }) => {
    ...
    return (
        <div className={`top-0 z-10 flex justify-center items-center h-12 ${selectedConversation?.messages?.length === 0 ? 'bg-transparent' : 'sticky bg-gradient-to-r from-[#F47A8A] to-[#6BB5E8] shadow-sm'}  py-2 px-4 text-sm text-white dark:border-none dark:bg-black dark:text-neutral-200`}>
            {
                ...
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mx-auto flex flex-col space-y-5 md:space-y-10 px-3 pt-5 md:pt-12 sm:max-w-[600px] text-center">
                  <div className="text-3xl font-bold text-[#1A1A18] dark:text-white">
                    {workflow} for PixelCraft
                  </div>
                  <div className="text-base text-[#777] dark:text-gray-400">
                    Review the bead map, ask for cleanup ideas, then return to publish.
                  </div>
                </div>
            } 
                        ...
                        <label className="flex items-center gap-2 cursor-pointer flex-shrink-0">
                            ...
                            <div
                                ...
                                className={`relative inline-flex h-5 w-10 items-center cursor-pointer rounded-full transition-colors duration-300 ease-in-out ${
                                    chatHistory ? 'bg-[#F47A8A] dark:bg-[#F47A8A]' : 'bg-gray-200'
                                }`}
                            >
                    ...
                    <div className="flex items-center gap-2 whitespace-nowrap">
                        <label className="flex items-center gap-2 cursor-pointer flex-shrink-0">
                            ...
                            <div
                                ...
                                className={`relative inline-flex h-5 w-10 items-center cursor-pointer rounded-full transition-colors duration-300 ease-in-out ${
                                    webSocketModeRef.current ? 'bg-[#6BB5E8] dark:bg-[#6BB5E8]' : 'bg-gray-200'
                                }`}
                            >
                                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300 ease-in-out ${
