...
export default function Home() {
  ...
  const [isPixelCraftMode, setIsPixelCraftMode] = useState<boolean>(false);
  const [pixelCraftLegendByColor, setPixelCraftLegendByColor] = useState<Record<string, PixelCraftLegendEntry>>({});
  ...
  const buildSyntheticImage = (pixelData: MappedPixel[][], dimensions: { N: number; M: number }): string => {
  ...
  const applyPixelCraftResult = useCallback((result: PixelCraftResult) => {
    const rows = Number(result.rows);
    const cols = Number(result.cols);
    if (!rows || !cols || !Array.isArray(result.cells)) return;

    const legendByColor: Record<string, PixelCraftLegendEntry> = {};
    (result.legend || []).forEach(entry => {
      if (entry.color_hex) {
        legendByColor[entry.color_hex.toUpperCase()] = {
          ...entry,
          color_hex: entry.color_hex.toUpperCase(),
        };
      }
    });

    const nextPixelData: MappedPixel[][] = Array.from({ length: rows }, () =>
    ...
    result.cells.forEach(cell => {
      ...
      const color = (cell.color_hex || '#FFFFFF').toUpperCase();
      const isExternal = Boolean(cell.empty) || color === '#FFFFFF';
      nextPixelData[row][col] = {
        key: isExternal ? 'TRANSPARENT' : (cell.symbol || color),
        color: isExternal ? '#FFFFFF' : color,
        isExternal,
      };
    });

    const nextColorCounts: { [key: string]: { count: number; color: string } } = {};
    let nextTotal = 0;
    nextPixelData.forEach(row => {
      row.forEach(cell => {
        if (!cell || cell.isExternal) return;
        const hexKey = cell.color.toUpperCase();
        if (!nextColorCounts[hexKey]) {
          nextColorCounts[hexKey] = { count: 0, color: hexKey };
        }
        nextColorCounts[hexKey].count++;
        nextTotal++;
      });
    });

    const dimensions = { N: cols, M: rows };
    setPixelCraftLegendByColor(legendByColor);
    ...
    setOriginalImageSrc(buildSyntheticImage(nextPixelData, dimensions));
    setGranularity(cols);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Unable to load PixelCraft editor payload:', error);
    }
  ...
  const handleSaveToPixelCraft = () => {
    ...
    mappedPixelData.forEach((row, rowIndex) => {
      row.forEach((cell, colIndex) => {
        const isEmpty = !cell || Boolean(cell.isExternal);
        const color = isEmpty ? '#FFFFFF' : cell.color.toUpperCase();
        if (!isEmpty) {
          if (!legendByColor[color]) {
            const generatedIndex = Object.keys(legendByColor).length + 1;
            legendByColor[color] = {
              symbol: `EDIT-${String(generatedIndex).padStart(2, '0')}`,
              name: `Edited Color ${generatedIndex}`,
              color_hex: color,
              confidence: 1,
              bbox: { x1: 0, y1: 0, x2: 0, y2: 0 },
            };
          }
          legendCounts[color] = (legendCounts[color] || 0) + 1;
        }
        cells.push({
          row: rowIndex + 1,
          col: colIndex + 1,
          symbol: isEmpty ? '' : legendByColor[color].symbol,
          color_hex: color,
          empty: isEmpty,
        });
      });
    ...
    const legend = Object.values(legendByColor)
      .filter(entry => legendCounts[entry.color_hex.toUpperCase()] > 0)
      .map(entry => ({
        ...entry,
        color_hex: entry.color_hex.toUpperCase(),
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className="relative flex flex-col items-center space-y-3">
              ...
              <div className="relative">
                <h1 className="relative text-4xl sm:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#F47A8A] via-[#9B72CF] to-[#6BB5E8] tracking-wider drop-shadow-2xl transform hover:scale-105 transition-transform duration-300">
                  PixelCraft
                </h1>
              ...
              <div className="relative">
                <h2 className="relative text-xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#6BB5E8] via-[#F47A8A] to-[#F5A623] tracking-widest transform hover:scale-102 transition-all duration-300">
                  拼豆精修工作台
                  <span className="text-xs font-normal text-gray-400 tracking-widest ml-1 align-middle">Gallery Editor</span>
                </h2>
